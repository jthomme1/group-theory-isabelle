theory DirProds
  imports Finprod Group_Hom Finite_Cyclic
begin

(* Indexed direct outer product of groups (possibly infinite). Basically an
   indexed version of DirProd. Probably pretty ugly to use. *)
...
proof(rule groupI)
  ...
  from DirProds_m_closed[OF x y assms] show m_closed: "x \<otimes>\<^bsub>DirProds G I\<^esub> y \<in> carrier (DirProds G I)" by blast
  fix z
  assume z: "z \<in> carrier (DirProds G I)"
  have "\<And>i. i \<in> I \<Longrightarrow> (x \<otimes>\<^bsub>DirProds G I\<^esub> y \<otimes>\<^bsub>DirProds G I\<^esub> z) i = (x \<otimes>\<^bsub>DirProds G I\<^esub> (y \<otimes>\<^bsub>DirProds G I\<^esub> z)) i"
  proof -
    fix i
    assume i: "i \<in> I"
    ...
    finally show "(x \<otimes>\<^bsub>DirProds G I\<^esub> y \<otimes>\<^bsub>DirProds G I\<^esub> z) i = (x \<otimes>\<^bsub>DirProds G I\<^esub> (y \<otimes>\<^bsub>DirProds G I\<^esub> z)) i" .
  qed
  thus "x \<otimes>\<^bsub>DirProds G I\<^esub> y \<otimes>\<^bsub>DirProds G I\<^esub> z = x \<otimes>\<^bsub>DirProds G I\<^esub> (y \<otimes>\<^bsub>DirProds G I\<^esub> z)" unfolding DirProds_def by auto
qed
...
proof -
  ...
  have "?r \<in> carrier (DirProds G I)" unfolding DirProds_def PiE_def Pi_def using assms(2, 3) ao by auto
...
proof -
  ...
  moreover with comp_mult[OF i] have "(inv\<^bsub>(DirProds G I)\<^esub> x \<otimes>\<^bsub>DirProds G I\<^esub> x) i = ((inv\<^bsub>(DirProds G I)\<^esub> x) i) \<otimes>\<^bsub>G i\<^esub> (x i)" by blast
...
proof -
  ...
  proof
    ...
    proof -
      obtain a where a[simp]: "a \<in> carrier (DirProds G I)" "a i = x" using DirProds_obtain_elem_carr[OF DP.is_group i x] .
      obtain b where b[simp]: "b \<in> carrier (DirProds G I)" "b i = y" using DirProds_obtain_elem_carr[OF DP.is_group i y] .
...
proof -
  ...
  proof(unfold_locales)
    ...
    proof(unfold DirProds_order[OF assms(2)])
      have "(order \<circ> G) i \<noteq> 0" if "i\<in>I" for i using assms(1)[OF that] by (simp add: finite_group.order_gt_0)
...
lemma (in comm_group) DirProds_two_iso:
  assumes "subgroup A G" "subgroup B G" "A \<noteq> B" "group.complementary_family G {A, B}" "carrier G = IDirProds G {A, B}"
  shows "(\<lambda>x. x A \<otimes> x B) \<in> iso (DirProds (\<lambda>H. G\<lparr>carrier := H\<rparr>) {A, B}) G" (is "?f \<in> iso ?H G")
proof -
  have "?f \<in> hom (DirProds (\<lambda>H. G\<lparr>carrier := H\<rparr>) {A, B}) G"
  proof (rule homI)
    fix x
    assume x: "x \<in> carrier ?H"
    then have xc: "x A \<in> carrier G" "x B \<in> carrier G" using comp_in_carr[OF x] assms(1, 2) subgroup.subset by(force)+
    thus "?f x \<in> carrier G" by blast
    fix y
    assume y: "y \<in> carrier ?H"
    then have yc: "y A \<in> carrier G" "y B \<in> carrier G" using comp_in_carr[OF y] assms(1, 2) subgroup.subset by(force)+
    have "(x \<otimes>\<^bsub>?H\<^esub> y) A \<otimes> (x \<otimes>\<^bsub>?H\<^esub> y) B = x A \<otimes> y A \<otimes> (x B \<otimes> y B)" by(auto simp: comp_mult)
    also have "\<dots> = x A \<otimes> x B \<otimes> (y A \<otimes> y B)" using m_assoc m_comm xc yc by simp
    finally show "?f (x \<otimes>\<^bsub>?H\<^esub> y) = ?f x \<otimes> ?f y" by blast
  qed
  then interpret f: group_hom ?H G ?f unfolding group_hom_def group_hom_axioms_def
  proof (safe)
    show "group ?H" by(intro DirProds_is_group; use assms(1, 2) subgroup.subgroup_is_group in blast)
  qed
  have "carrier G \<subseteq> ?f ` carrier ?H"
  proof -
    have "carrier G = A <#> B" using assms(5) unfolding IDirProds_def using generate_subgroup_eq_set_mult[OF assms(1, 2)] by simp
    also have "\<dots> \<subseteq> ?f ` carrier ?H"
    proof
      fix x
      assume "x \<in> A <#> B"
      then obtain a b where ab: "a \<in> A" "b \<in> B" "x = a \<otimes> b" unfolding set_mult_def by blast
      let ?y = "(\<lambda>H. undefined)(A := a, B := b)"
      have "?y \<in> carrier ?H"
        unfolding DirProds_def PiE_def Pi_def extensional_def using ab by simp
      moreover have "(\<lambda>x. x A \<otimes> x B) ?y = x" using ab assms(3) by simp
      ultimately show "x \<in> ?f ` carrier ?H" by blast
    qed
    finally show ?thesis .
  qed
  moreover have "\<forall>x\<in>carrier ?H. ?f x = \<one> \<longrightarrow> x = \<one>\<^bsub>?H\<^esub>"
  proof (rule, rule)
    fix x
    assume x: "x \<in> carrier ?H" "x A \<otimes> x B = \<one>"
    then have xs: "x A \<in> A" "x B \<in> B" using comp_in_carr[OF x(1)] by simp_all
    then have "x A \<in> carrier G" "x B \<in> carrier G" using assms(1,2) subgroup.subset by blast+
    from comm_inv_char[OF this x(2)] have "inv x A = x B" .
    then have "x B \<in> A" using subgroup.m_inv_closed[OF assms(1) xs(1)] by simp
    then have "x B \<in> (A \<inter> B)" using xs(2) by blast
    moreover have "A \<inter> B = {\<one>}" using assms(1-4)
      unfolding complementary_family_def complementary_def IDirProds_def
      using generate_subgroup_id by simp
    ultimately have "x B = \<one>" by simp
    moreover then have "x A = \<one>" using x \<open>x A \<in> carrier G\<close> by simp
    ultimately show "x = \<one>\<^bsub>?H\<^esub>" using DirProds_one[OF x(1)] assms(1, 2) subgroup.subgroup_is_group by auto
  qed
  ultimately show "?f \<in> iso ?H G" using f.iso_iff by blast
qed

lemma (in comm_group) subgroup_iso_DirProds_IDirProds:
  assumes "subgroup J G" "J = IDirProds G Hs"
  and "finite Hs" "\<And>H. H \<in> Hs \<Longrightarrow> subgroup H (G\<lparr>carrier := J\<rparr>)" "complementary_family Hs"
  shows "(\<lambda>x. \<Otimes>\<^bsub>G\<^esub>H\<in>Hs. x H) \<in> iso (DirProds (\<lambda>H. G\<lparr>carrier := H\<rparr>) Hs) (G\<lparr>carrier := J\<rparr>)" (is "?fp \<in> iso ?DP ?J")
proof -
  interpret J: comm_group ?J using subgroup_is_comm_group[OF assms(1)] .
  interpret DP: comm_group ?DP
    by(intro DirProds_is_comm_group; use J.subgroup_is_comm_group[OF assms(4)] in simp)
  have inJ: "H \<subseteq> J" if "H \<in> Hs" for H using subgroup.subset[OF assms(4)[OF that]] by simp
  have hom: "?fp \<in> hom ?DP ?J"
  proof (rule homI)
    fix x
    assume x[simp]: "x \<in> carrier ?DP"
    show "finprod G x Hs \<in> carrier ?J"
    proof(subst finprod_subgroup[OF _ assms(1)])
      show "x \<in> Hs \<rightarrow> J" using inJ comp_in_carr[OF x] by auto
      thus "finprod ?J x Hs \<in> carrier ?J" by(intro J.finprod_closed; simp)
    qed
    fix y
    assume y[simp]: "y \<in> carrier ?DP"
    show "finprod G (x \<otimes>\<^bsub>?DP\<^esub> y) Hs = finprod G x Hs \<otimes>\<^bsub>?J\<^esub> finprod G y Hs"
    proof(subst (1 2 3) finprod_subgroup[of _ _ J])
      show xyJ: "x \<in> Hs \<rightarrow> J" "y \<in> Hs \<rightarrow> J" using x y inJ comp_in_carr[OF x] comp_in_carr[OF y] by auto
      show xyJ1: "x \<otimes>\<^bsub>?DP\<^esub> y \<in> Hs \<rightarrow> J" using inJ x y comp_in_carr[of "x \<otimes>\<^bsub>?DP\<^esub> y"] by fastforce
      show "subgroup J G" using assms(1) .
      show "finprod ?J (x \<otimes>\<^bsub>?DP\<^esub> y) Hs = finprod ?J x Hs \<otimes>\<^bsub>?J\<^esub> finprod ?J y Hs"
      proof (rule J.finprod_eq)
        show "x \<in> Hs \<rightarrow> carrier ?J" "y \<in> Hs \<rightarrow> carrier ?J" using xyJ by simp_all
        show "x \<otimes>\<^bsub>?DP\<^esub> y \<in> Hs \<rightarrow> carrier ?J" using xyJ1 by simp
        fix H
        assume H: "H \<in> Hs"
        then have "x H \<otimes>\<^bsub>G\<lparr>carrier := H \<rparr>\<^esub> y H = (x \<otimes>\<^bsub>?DP\<^esub> y) H"
          by(intro comp_mult[symmetric])
        thus "x H \<otimes>\<^bsub>?J\<^esub> y H = (x \<otimes>\<^bsub>?DP\<^esub> y) H" by simp
      qed
    qed
  qed
  then interpret fp: group_hom ?DP ?J ?fp unfolding group_hom_def group_hom_axioms_def by blast
  have s: "subgroup H G" if "H \<in> Hs" for H using incl_subgroup[OF assms(1) assms(4)[OF that]] .
  have "kernel ?DP ?J ?fp = {\<one>\<^bsub>?DP\<^esub>}"
  proof -
    have "a = \<one>\<^bsub>?DP\<^esub>" if "a \<in> kernel ?DP ?J ?fp" for a
    proof -
      from that have a: "finprod G a Hs = \<one>" "a \<in> carrier ?DP" unfolding kernel_def by simp_all
      from comp_fam_imp_triv_finprod[OF assms(5, 3) s a(1)] comp_in_carr[OF a(2)] have "\<forall>H\<in>Hs. a H = \<one>" by simp
  ...
  moreover have "J \<subseteq> ?fp ` carrier ?DP"
    using assms(2) IDirProds_eq_finprod_PiE[OF assms(3) incl_subgroup[OF assms(1) assms(4)]]
...
lemma (in comm_group) iso_DirProds_IDirProds:
  ...
  shows "(\<lambda>x. \<Otimes>\<^bsub>G\<^esub>H\<in>Hs. x H) \<in> iso (DirProds (\<lambda>H. G\<lparr>carrier := H\<rparr>) Hs) G"
proof -
  from assms(1) is_idirprod.simps[of "carrier G" Hs]
  have "carrier G = IDirProds G Hs" "\<And>x. x \<in> Hs \<Longrightarrow> x \<lhd> G" "complementary_family Hs" by blast+
  from subgroup_iso_DirProds_IDirProds[OF subgroup_self this(1) assms(2) normal_imp_subgroup this(3)] this(2)
  show ?thesis by auto
qed

lemma (in comm_group) cong_DirProds_IDirProds:
  assumes "is_idirprod (carrier G) Hs" "finite Hs"
  shows "DirProds (\<lambda>H. G\<lparr>carrier := H\<rparr>) Hs \<cong> G"
...
proof (intro isoI homI)
  ...
  proof (unfold bij_betw_def, rule)
    show "inj_on (\<lambda>x. x G) (carrier (DirProds f {G}))" by(intro inj_onI, unfold DirProds_def PiE_def Pi_def extensional_def, fastforce)
    show "(\<lambda>x. x G) ` carrier (DirProds f {G}) = carrier (f G)"
    proof(intro equalityI subsetI)
      show "x \<in> carrier (f G)" if "x \<in> (\<lambda>x. x G) ` carrier (DirProds f {G})" for x using that unfolding DirProds_def by auto
...
proof (intro isoI homI)
  ...
  proof (unfold bij_betw_def, rule)
    show "inj_on (\<lambda>x. (\<lambda>_\<in>{G}. x)) (carrier (f G))" by(intro inj_onI, metis imageI image_constant image_restrict_eq member_remove remove_def)
    show "(\<lambda>x. (\<lambda>_\<in>{G}. x)) ` carrier (f G) = carrier (DirProds f {G})" unfolding DirProds_def by fastforce
  qed
...
lemma DirProds_iso:
  assumes "bij_betw f I J" "\<And>i. i\<in>I \<Longrightarrow> Gs i \<cong> Hs (f i)" "\<And>i. i\<in>I \<Longrightarrow> group (Gs i)" "\<And>j. j\<in>J \<Longrightarrow> group (Hs j)"
...
proof -
  ...
  proof(intro eq_parts_imp_eq[OF hc[OF DG.m_closed[OF that]] DH.m_closed[OF hc[OF that(1)] hc[OF that(2)]]])
    ...
    proof(subst comp_mult)
      show "(if j \<in> J then h (g j) (x (g j) \<otimes>\<^bsub>Gs (g j)\<^esub> y (g j)) else undefined) = (?h x \<otimes>\<^bsub>DirProds Hs J\<^esub> ?h y) j"
      proof(subst comp_mult)
        ...
        thus "(if j \<in> J then h (g j) (x (g j) \<otimes>\<^bsub>Gs (g j)\<^esub> y (g j)) else undefined) =
              (if j \<in> J then h (g j) (x (g j)) else undefined) \<otimes>\<^bsub>Hs j\<^esub> (if j \<in> J then h (g j) (y (g j)) else undefined)"
          using j by simp
  ...
  proof
    ...
    proof -
      ...
      hence kiso: "\<And>i. i\<in>I \<Longrightarrow> k i \<in> iso (Hs (f i)) (Gs i)" using h by (simp add: assms(3) group.iso_set_sym)
      ...
      proof -
        ...
        thus ?thesis using h[OF gj, unfolded iso_def] k[OF gj] that fgi(2)[OF that(1)] bij_betw_inv_into_right
      ...
      proof (cases "j \<in> J")
        case True
        thus ?thesis using hk[OF True comp_in_carr[OF that True]] fgi(2)[OF True] g[unfolded bij_betw_def] by auto
      next
        case False
      ...
      proof -
        have "?y i \<in> carrier (Gs i)" if i: "i \<in> I" for i
          using k[OF i] h[OF i] comp_in_carr[OF xc] assms(1) bij_betwE iso_in_carr kiso that by fastforce
    ...
    qed
  qed
  moreover have "x = \<one>\<^bsub>DirProds Gs I\<^esub>" if "x \<in> carrier (DirProds Gs I)" "?h x = \<one>\<^bsub>DirProds Hs J\<^esub>" for x
  proof -
    ...
    proof
      ...
      from DirProds_one[OF this] that(2) have "(if j \<in> J then h (g j) (x (g j)) else undefined) = \<one>\<^bsub>Hs j\<^esub>" if "j \<in> J" for j
        using that by blast
...
proof -
  ...
  proof -
    ...
    proof -
      from J that have "(J i) ` (carrier (Gs i)) = carrier ((f \<circ> Gs) i)" unfolding iso_def bij_betw_def by blast
      hence "\<exists>y. y \<in> carrier (Gs i) \<and> (J i) y = x i" using that by (metis comp_in_carr imageE)
      with someI_ex[OF this] that show "(J i) ((K i) (x i)) = x i" using K J K_iso unfolding inv_into_def by auto
    qed
  ...
  proof (intro homI)
    ...
    show "?r (x \<otimes>\<^bsub>DirProds Gs I\<^esub> y) = ?r x \<otimes>\<^bsub>DirProds (f \<circ> Gs) I\<^esub> ?r y" if "x \<in> carrier (DirProds Gs I)" "y \<in> carrier (DirProds Gs I)" for x y
      using that J comp_in_carr[OF that(1)] comp_in_carr[OF that(2)] unfolding DirProds_def iso_def hom_def by force
  qed
  ...
  proof
    ...
    qed
  qed
  moreover have "x = \<one>\<^bsub>DirProds Gs I\<^esub>" if "x \<in> carrier (DirProds Gs I)" "?r x = \<one>\<^bsub>DirProds (f \<circ> Gs) I\<^esub>" for x
  proof -
    have "\<forall>i\<in>I. x i = \<one>\<^bsub>Gs i\<^esub>"
    proof
      fix i
      assume i: "i \<in> I"
      with J assms interpret Ji: group_hom "(Gs i)" "(f \<circ> Gs) i" "J i" unfolding group_hom_def group_hom_axioms_def iso_def by blast
      from that have rx: "?r x \<in> carrier (DirProds (f \<circ> Gs) I)" by simp
      from i DirProds_one[OF this] that have "(\<lambda>i\<in>I. (if i \<in> I then J i else (\<lambda>_. undefined)) (x i)) i = \<one>\<^bsub>(f \<circ> Gs) i\<^esub>" by blast
      hence "(J i) (x i) = \<one>\<^bsub>(f \<circ> Gs) i\<^esub>" using i by simp
      with Ji.iso_iff mp[OF spec[OF J[unfolded Ball_def]] i] comp_in_carr[OF that(1) i] show "x i = \<one>\<^bsub>Gs i\<^esub>" by fast
    qed
...
proof (intro is_isoI isoI)
  ...
  proof(intro homI)
    show "?f a \<in> carrier ?R" if "a \<in> carrier ?L" for a using that unfolding DirProds_def PiE_def Pi_def extensional_def by auto
  ...
  proof (intro bij_betwI)
    ...
    show "?f (?g x) = x" if "x \<in> carrier ?R" for x using that unfolding DirProds_def PiE_def Pi_def extensional_def by auto
    show "?g (?f x) = x" if "x \<in> carrier ?L" for x using that unfolding DirProds_def PiE_def Pi_def extensional_def by force
  qed
qed
...
proof (cases "n = 1")
  case True
  show ?thesis by (intro triv_iso[OF group_integer_mod_group DirProds_is_group], use DirProds_empty carrier_integer_mod_group True in auto)
next
  case nno: False
  interpret DP: group ?DP by (intro DirProds_is_group, use group_integer_mod_group in blast)
  have "order ?DP = prod (order \<circ> (\<lambda>p. Z (p ^ multiplicity p n))) (prime_factors n)" by (intro DirProds_order, blast)
  ...
  then interpret DP: finite_group ?DP by (unfold_locales, unfold order_def, metis assms card.infinite)
  let ?f = "\<lambda>p\<in>(prime_factors n). 1"
  have fc: "?f \<in> carrier ?DP"
  proof -
    have p: "0 < multiplicity p n" if "p \<in> prime_factors n" for p using prime_factors_multiplicity that by auto
  ...
  proof -
    ...
    proof (intro pairwise_coprime_dvd'[OF _ _ n[symmetric]])
      ...
      proof 
        ...
        proof -
          from j have "(?f [^]\<^bsub>?DP\<^esub> j) p = 0" using that unfolding DirProds_def one_integer_mod_group by auto
          ...
          moreover have "group.ord (Z (p ^ multiplicity p n)) (?f p) = p ^ multiplicity p n"
            by (metis (no_types, lifting) Zn_neq1_cyclic_group Zn_order comp_in_carr cyclic_group.ord_gen_is_group_order fc integer_mod_group_1 restrict_apply' that)
...
proof -
  ...
  proof(subst cp, intro DirProds_iso2)
    show "inj_on (\<lambda>p. p ^ multiplicity p n) (prime_factors n)" by (intro inj_onI; simp add: prime_factors_multiplicity prime_power_inj'')
    show "group (DirProds Z ((\<lambda>p. p ^ multiplicity p n) ` prime_factors n))" by (intro DirProds_is_group, use group_integer_mod_group in auto)
  qed
